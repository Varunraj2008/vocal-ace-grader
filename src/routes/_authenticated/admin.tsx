...
export const Route = createFileRoute("/_authenticated/admin")({
  head: () => ({
    meta: [
      { title: "Admin — Vocalis" },
      { name: "description", content: "Organization admin dashboard for assessments and users." },
      { property: "og:title", content: "Admin — Vocalis" },
      { property: "og:description", content: "Organization admin dashboard." },
    ],
  }),
  component: AdminPage,
});
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-left text-muted-foreground">
                <tr><th className="py-2">User</th><th>Email</th><th>Sessions</th><th>Best</th><th>Last completed</th></tr>
              </thead>
              <tbody className="divide-y">
                {(u.data ?? []).map((row: any) => (
                  <tr key={row.id}>
                    ...
                    <td>{row.stats.best ? <Badge className="bg-brand-gradient text-brand-foreground border-0">{Number(row.stats.best).toFixed(1)}</Badge> : "—"}</td>
